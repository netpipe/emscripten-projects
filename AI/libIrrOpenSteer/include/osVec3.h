...
#include <irrlicht.h>
#include "osUtilities.h"  // for interpolate, etc.

//! namespace OpenSteer
namespace OpenSteer{

  //! OpenSteer's generic type for 3d vectors
  ...
  class Vec3{
    public:
      ...
      //! constructor
      Vec3(irr::core::vector3df v): x( v.X ), y( v.Y ), z( v.Z ) {}

      //! convert to Irrlicht-Vector
      ...
      /*! \note - use @c distance from the Vec3Utilitites header instead. */
      ...
      /*! If the vector is shorter its value is returned unaltered, if the
          vector is longer the value returned has length of maxLength
          and is paralle to the original input. */
      ...
      Vec3 sphericalWrapAround(const Vec3& center, float _radius){
        const Vec3 offset = *this - center;
        const float r = offset.length();
        if(r > _radius)
          return *this + ((offset/r) * _radius * -2);
        else
          return *this;
      }

      //! name for frequently used zero vector constant
      static const Vec3 zero;
      //! name for frequently used side vector constant
      static const Vec3 side;
      //! name for frequently used up vector constant
      static const Vec3 up;
      //! name for frequently used forward vector constant
  ...
  //! Returns a position randomly distributed inside a sphere of unit radius centered at the origin.
  /*! Orientation will be random and length will range between 0 and 1 */
  Vec3 RandomVectorInUnitRadiusSphere(void);

  //! Returns a position randomly distributed on a disk of unit radius on the XZ (Y=0) plane, centered at the origin.
  /*! Orientation will be random and length will range between 0 and 1 */
  Vec3 randomVectorOnUnitRadiusXZDisk(void);

  //! Returns a position randomly distributed on the surface of a sphere of unit radius centered at the origin.
  /*! Orientation will be random and length will be 1 */
  inline Vec3 RandomUnitVector(void){
    return RandomVectorInUnitRadiusSphere().normalize();
  }

  //! Returns a position randomly distributed on a circle of unit radius on the XZ (Y=0) plane, centered at the origin.
  /*! Orientation will be random and length will be 1 */
  ...
  //! Enforce an upper bound on the angle by which a given arbitrary vector diviates from a given reference direction (specified by a unit basis vector).
  ...
  //! Enforce a lower bound on the angle by which a given arbitrary vector diviates from a given reference direction (specified by a unit basis vector).
  ...
  /*! The line is defined in terms of a point on the line ("lineOrigin") and a UNIT vector parallel to the line ("lineUnitTangent") */
  ...
  //! given a vector, return a vector perpendicular to it (note that this arbitrarily selects one of the infinitude of perpendicular vectors)
