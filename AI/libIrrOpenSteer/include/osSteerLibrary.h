...
namespace OpenSteer {
  ...
  //! SteerLibraryMixin
  /*! This mixin (class with templated superclass) adds the "steering library" functionality to a given base class.  SteerLibraryMixin assumes its base class supports the AbstractVehicle interface. */
  ...
  class SteerLibraryMixin : public Super{
    public:
      ...
      //! proposed, experimental new Seek behavior
      Vec3 steerForSeekEx (const Vec3& target);
      //! Flee behavior
      Vec3 steerForFlee (const Vec3& target);
      //! proposed, experimental new Flee behavior
      Vec3 steerForFleeEx (const Vec3& target);

      // Follow Path
      //! Path Following behavior
      Vec3 steerToFollowPath (const int direction, const float predictionTime, Pathway& path);
      //! Path Following (exact) behavior
      ...
      // Neighbors
      //! Unaligned collision avoidance behavior avoid colliding with other nearby vehicles moving in unconstrained directions.
      /*! Determine which (if any) other other vehicle we would collide
          with first, then steers to avoid the site of that potential collision.
          Returns a steering force vector, which is zero length if there
          is no impending collision. */
      ...
      // globals only for the sake of graphical annotation
      ...
      /*! Does a hard steer away from any other agent who comes withing a critical distance.
          Ideally this should be replaced with a call to steerForSeparation. */
      Vec3 steerToAvoidCloseNeighbors (const float minSeparationDistance, const AVGroup& others);
      ...
      // XXX ("utility vehicle"?)
      //! cwr experimental 9-9-02 -- name OK?
      ...
      //! called when steerToAvoidObstacles decides steering is required (default action is to do nothing, layered classes can overload it)
      virtual void annotateAvoidObstacle (const float /*minDistanceToCollision*/){}
      //! called when steerToFollowPath decides steering is required (default action is to do nothing, layered classes can overload it)
      virtual void annotatePathFollowing (const Vec3& /*future*/,
                                          const Vec3& /*onPath*/,
                                          const Vec3& /*target*/,
                                          const float /*outside*/){}
      //! called when steerToAvoidCloseNeighbors decides steering is required (default action is to do nothing, layered classes can overload it)
      virtual void annotateAvoidCloseNeighbor (const AbstractVehicle& /*other*/,
                                               const float /*additionalDistance*/){}
      //! called when steerToAvoidNeighbors decides steering is required (default action is to do nothing, layered classes can overload it)
...
//! proposed, new flee
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerForFleeEx (const Vec3& target){
...
//! proposed, new seek
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerForSeekEx (const Vec3& target){
...
//! Path Following behavior
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerToFollowPath (const int direction, const float predictionTime, Pathway& path){
...
/*! Returns a steering force to avoid a given obstacle.  The purely lateral
    ...
    XXX method.  This may well change.<br>
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerToAvoidObstacle (const float minTimeToCollision, const Obstacle& obstacle){
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerToAvoidObstacles (const float minTimeToCollision, const ObstacleGroup& obstacles){
...
/*! avoid colliding with other nearby
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerToAvoidNeighbors (const float minTimeToCollision, const AVGroup& others){
...
template<class Super> float OpenSteer::SteerLibraryMixin<Super>
...
//! Given the time until nearest approach (predictNearestApproachTime) determine position of each vehicle at that time, and the distance between them
template<class Super> float OpenSteer::SteerLibraryMixin<Super>
::computeNearestApproachPositions (AbstractVehicle& otherVehicle, float time){
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerToAvoidCloseNeighbors (const float minSeparationDistance, const AVGroup& others){
...
template<class Super> bool OpenSteer::SteerLibraryMixin<Super>
::inBoidNeighborhood (const AbstractVehicle& otherVehicle, const float minDistance, const float maxDistance, const float cosMaxAngle){
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerForSeparation (const float maxDistance, const float cosMaxAngle, const AVGroup& flock){
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerForAlignment (const float maxDistance, const float cosMaxAngle, const AVGroup& flock){
    // steering accumulator and count of neighbors, both initially zero
    Vec3 steering;
    int neighbors = 0;
...
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>::steerForCohesion (const float maxDistance, const float cosMaxAngle, const AVGroup& flock){
    // steering accumulator and count of neighbors, both initially zero
    Vec3 steering;
    int neighbors = 0;
...
//! pursuit of another vehicle with ceiling on prediction time
template<class Super> OpenSteer::Vec3 OpenSteer::SteerLibraryMixin<Super>
::steerForPursuit (const AbstractVehicle& quarry, const float maxPredictionTime){
