...
void Reset(void)
{
 // Fill RAM with 000
  ...
  {
    disk[2] = open(Interface.GetBIOSFilename(), O_BINARY |  O_RDWR);//O_NOINHERIT
  }

  if (Interface.GetFDImageFilename() != NULL)
  {
    disk[1] = open(Interface.GetFDImageFilename(), O_BINARY |  O_RDWR);
  }

  if (Interface.GetHDImageFilename() != NULL)
  {
    disk[0] = open(Interface.GetHDImageFilename(), O_BINARY |  O_RDWR);
  }
			...
			OPCODE 50: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					...
					OPCODE 1: // GET_RTC
						...
						CAST(short)mem[SEGREG_OP(REG_ES, REG_BX, 36+)] = clock();
					OPCODE 2: // DISK_READ
