...
void RefreshScreen(void *Buffer,int Width,int Height)
{
  ...
  /* Show replay icon */
  RPLShow(&ScrBuf,VideoX+10,VideoY+10);
  ...
  if(Key&CON_RELEASE)
    ...
    switch(Key&CON_KEYCODE)
    {
      ...
      case XK_F7:
        ...
        break;
      case XK_F8:
        if(Key&CON_SHIFT) { RPLPlay(RPL_OFF);RPLRecord(RPL_TOGGLE); }
        else              { RPLRecord(RPL_OFF);RPLPlay(RPL_RESET); }
        break;
      case XK_F9:
      case XK_Page_Up:
        if(!FastForward)
