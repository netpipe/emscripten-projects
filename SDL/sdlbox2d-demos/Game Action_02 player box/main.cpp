...
public:
	...
	GroundS(float x, float y, int w, int h, int ang){
    setWall(x,y,w,h,ang);
 	}
...
public:
	...
	BoxD(int var, int x,int y,int w,int h,int ang){
    //Выбор и создание текстуры
    switch (var)
        {
        case a:
...
public:
	...
	switch (var)
	{
     case a:
            setBullet(x, y, vx, vy);
            text = loadImg ("images/bullet.png");
     ...
     case b:
            ...
            text = loadImg ("images/bullet2.png");
...
while (!quit)
{
    ...
    lvl.Draw(renderer);
		...
		if (KEY_UP && onGround)         {playerBody->ApplyLinearImpulse(b2Vec2 (0,-100), b2Vec2 ( 0, 0) ,  1);};
