...
/*static*/ int getInstanceVarInfo(StackHandler & sa,VarRefPtr & vr,SQUserPointer & data) {
  ...
  if (!(vr->m_access & (VAR_ACCESS_STATIC|VAR_ACCESS_CONSTANT))) {
    ...
    ClassTypeBase *ctb_var = (ClassTypeBase*)vr->instanceType;
    ClassTypeBase *ctb_inst = (ClassTypeBase*)typetag;
    up = (char *)instance.GetInstanceUP(0);
    // Walk base classes for instance until type tag for var matches, 
    // also adjust for inheritence offset
    while(ctb_inst && ctb_var!=ctb_inst) {
      up = (char*)up - ctb_inst->m_offset;
      ctb_inst = ctb_inst->m_pbase;
    }
    if (!ctb_inst) {
      throw SquirrelError(_SC("Invalid Instance Type"));
    }
...
/*static*/ int getVar(StackHandler & sa,VarRef * vr,void * data) {
  ...
  case VAR_TYPE_INSTANCE_PTR:
    {
      void*    ptr = *(uintptr_t**)data; // use the actual address of instance
      if (!CreateNativeClassInstance(sa.GetVMPtr(),vr->varType->GetTypeName(),ptr,0)) { // ptr = address. Allocates memory.
        SCSNPRINTF(g_msg_throw.s,sizeof(g_msg_throw),_SC("getVar(): Could not create instance: %s"),vr->varType->GetTypeName());
        throw SquirrelError(g_msg_throw.s);
      } // if
    }
